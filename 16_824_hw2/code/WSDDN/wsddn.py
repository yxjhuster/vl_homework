...
class WSDDN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False, training=True):
        ...
        # DONE: Define the WSDDN model (look at faster_rcnn.py)
    ...
    def forward(self,
        ...
        # DONE: Use im_data and rois as input
        # compute cls_prob which are N_roi X 20 scores
        # Checkout faster_rcnn.py for inspiration
        feat = self.features(im_data)
        pooled_features = self.roi_pool(feat, rois)
        x = pooled_features.view(pooled_features.size()[0], -1)
        # pdb.set_trace()
        x = self.classifier(x)
        cls_score = self.score_fc(x)
        bbox_score = self.bbox_fc(x)
        cls_prob = cls_score * bbox_score
    ...
    def build_loss(self, cls_prob, label_vec):
        ...
        """
        # DONE: Compute the appropriate loss using the cls_prob that is the
        # output of forward()
        # Checkout forward() to see how it is called
        cls_prob_sum = torch.sum(cls_prob, dim=0, keepdim=True)
        cls_prob_sum = torch.clamp(cls_prob_sum, 0, 1)
        #  to avoid multi-prediction we need to clip the value
        bceloss = self.criterion(cls_prob_sum, label_vec)
    ...
    def detect(self, image, rois, thr=0.3):
        ...
        pred_boxes, scores, classes = \
            self.interpret_faster_rcnn(
                cls_prob, rois, im_info, image.shape, min_score=thr)
