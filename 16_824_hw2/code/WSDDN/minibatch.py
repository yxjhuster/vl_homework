    ...
    for im_i in xrange(num_images):
        labels, overlaps, im_rois, bbox_targets, bbox_inside_weights \
            = _sample_rois(roidb[im_i], fg_rois_per_image, rois_per_image,
                           num_classes)
        # DONE: same as get_minibatch, but we only use the image-level labels
        ...
        labels = np.nonzero(np.unique(roidb[im_i]['gt_classes']))[0]
        labels_one_hot = np.sum(
            np.eye(num_classes, dtype=np.float32)[labels - 1], axis=0)
        labels_blob = np.vstack([labels_blob, labels_one_hot])
